...
class Threshold:
    ...
    def evaluate(self, value: float) -> str:
        ...
        if self.inverted:
            ...
            elif value <= self.bad: return "WARNING"
            else: return "BAD"
        else:
            if value >= self.good: return "GOOD"
            elif value >= self.warning: return "WARNING"
...
class ContextualHealthResult:
    ...
    def print_report(self):
        print(f"\n  {'='*65}")
        print(f"  CONTEXTUALIZED HEALTH — Giai đoạn 3")
        print(f"  {'='*65}")
        ...
        for m in sorted(self.metrics.values(), key=lambda x: x.score):
            ...
            print(f"  {ico.get(m.interpretation, '⚪')} {m.name:<26} {val_str:>8} "
                  f"{m.score:>6.2f} {th_str:>14} {base_str:>10}")
        print(f"  {'─'*65}")
...
class ContextualHealthEngine:
    ...
    def assess(self, symbol: str) -> Optional[ContextualHealthResult]:
    ...
    def assess_many(self, symbols: List[str]) -> Dict[str, Optional[ContextualHealthResult]]:
    ...
    def _adjust_threshold(self, metric: str, base: Threshold, arch: BusinessArchetype) -> Threshold:
        ...
        for attr_val, rule in rules.items():
            ...
            if isinstance(attr_val, Enum):
                ...
                for dna_attr in [arch.revenue_model, arch.operating_leverage,
                                  arch.financial_leverage, arch.cyclicality,
                                  arch.pricing_power]:
    ...
    def _load_ratios(self, symbol: str) -> Optional[Dict[str, float]]:
        ...
        try:
            ...
            rows = conn.execute(
                "SELECT ratio_name, ratio_value FROM health_ratios "
...
def print_contextual_report(results: Dict[str, Optional[ContextualHealthResult]]):
    print(f"\n  {'='*65}")
    print(f"  CONTEXTUALIZED COMPANY HEALTH — Giai đoạn 3 (Archetype-Aware)")
    print(f"  {'='*65}")
