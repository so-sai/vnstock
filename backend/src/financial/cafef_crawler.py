...
class CafeFCrawler:
    ...
    def fetch_statement(self, symbol: str, st_type: int, year: int, quarter: int) -> dict[str, float]:
        ...
        for url in urls:
            ...
            if not self.use_playwright:
                ...
                try:
                    ...
                    if resp.status_code == 200:
                        html_raw = resp.text
                except Exception as e:  # noqa: BLE001 - fallback ladder: requests fail → thử Playwright
            ...
            except Exception as e:  # noqa: BLE001 - fallback ladder: parse URL này fail → thử URL/method khác
    ...
    def _parse_cafef_bank_api(self, symbol: str, entity_type: str, html: str) -> list[dict]:
        ...
        except Exception as e:  # noqa: BLE001 - external provider resilience: parse lỗi → trả [] không chặn các mã khác
    ...
    def fetch_cafef_cashflow(self, symbol: str) -> list[dict]:
        ...
        for year, qtr in pages:
            ...
            except Exception as e:  # noqa: BLE001 - batch isolation: 1 quý lỗi không dừng các quý khác
    ...
    def fetch_vci_bridge(self, symbol: str) -> list[dict]:
        ...
        except Exception as e:  # noqa: BLE001 - external provider resilience: VCI API fail → trả []
    ...
    def fetch_vndirect_api(self, symbol: str) -> list[dict]:
        ...
        try:
            ...
            if r.status_code != 200:
                ...
                return []
            payload = r.json()
        except Exception as e:  # noqa: BLE001 - external provider resilience: VNDirect fail → trả []
        ...
        for tcbs_type, tag in sections.items():
            ...
            try:
                ...
                if r.status_code != 200:
                    ...
                    continue
                payload = r.json()
            except Exception as e:  # noqa: BLE001 - batch isolation: 1 section TCBS lỗi không dừng các section khác
    ...
    def fetch_note_indicator(self, symbol: str) -> list[dict]:
        ...
        except Exception as e:  # noqa: BLE001 - external provider resilience: NoteIndicator fail → trả []
    ...
    def _try_cafef_pw(url: str) -> str | None:
        ...
        except Exception as e:  # noqa: BLE001 - external provider resilience: Playwright fail → trả None
