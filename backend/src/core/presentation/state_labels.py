...
RISK_LABELS: dict[str, PresentationLabel] = {
    "LOCKDOWN": PresentationLabel(
        ...
        color="red", icon="shield-off",
        priority=5, severity_score=0.95,
        action_bias="REDUCE_RISK",
    ),
    "HIGH_STRESS": PresentationLabel(
        label_vi="Áp lực rủi ro cao",
        color="orange", icon="alert-circle",
        priority=4, severity_score=0.80,
        action_bias="REDUCE_RISK",
    ),
    "DEFENSIVE": PresentationLabel(
        label_vi="Phòng thủ",
        color="yellow", icon="shield",
        priority=3, severity_score=0.60,
        action_bias="HOLD",
    ),
    "NEUTRAL": PresentationLabel(
        label_vi="Trung tính",
        color="gray", icon="minus",
        priority=2, severity_score=0.30,
        action_bias="HOLD",
    ),
    "RISK_ON": PresentationLabel(
        label_vi="Chấp nhận rủi ro",
        color="green", icon="activity",
        priority=1, severity_score=0.15,
        action_bias="BUY",
    ),
}
...
LIQUIDITY_LABELS: dict[str, PresentationLabel] = {
    ...
    "FRAGMENTED": PresentationLabel(
        ...
        color="yellow", icon="git-branch",
        priority=3, severity_score=0.5,
        action_bias="WAIT",
    ),
    "CONTRACTION": PresentationLabel(
        label_vi="Thanh khoản co hẹp",
        color="orange", icon="trending-down",
        priority=4, severity_score=0.7,
        action_bias="REDUCE_RISK",
    ),
}
...
MARKET_PHASE_LABELS: dict[str, PresentationLabel] = {
    "BULL_TRENDING": PresentationLabel(
        label_vi="Xu hướng tăng", color="green", priority=1,
        severity_score=0.1, action_bias="BUY",
    ),
    "BEAR_TRENDING": PresentationLabel(
        label_vi="Xu hướng giảm", color="red", priority=4,
        severity_score=0.7, action_bias="REDUCE_RISK",
    ),
    "SIDEWAYS": PresentationLabel(
        label_vi="Đi ngang tích lũy", color="gray", priority=2,
        severity_score=0.3, action_bias="HOLD",
    ),
    "PHAN_HOA": PresentationLabel(
        label_vi="Phân hóa mạnh", color="yellow", priority=3,
        severity_score=0.6, action_bias="WAIT",
    ),
    "CRISIS": PresentationLabel(
        label_vi="Khủng hoảng", color="red", priority=5,
        severity_score=0.95, action_bias="REDUCE_RISK",
    ),
}
...
ENTROPY_LABELS: dict[str, PresentationLabel] = {
    "HỘI_TỤ": PresentationLabel(
        label_vi="Tín hiệu hội tụ", color="green",
        priority=1, severity_score=0.1, action_bias="HOLD",
    ),
    "PHÂN_KỲ": PresentationLabel(
        label_vi="Tín hiệu phân kỳ", color="yellow",
        priority=3, severity_score=0.5, action_bias="WAIT",
    ),
    "NHIỄU_CAO": PresentationLabel(
        label_vi="Nhiễu thị trường cao", color="orange",
        priority=4, severity_score=0.7, action_bias="WAIT",
    ),
    "MẤT_ỔN_ĐỊNH": PresentationLabel(
        label_vi="Mất ổn định nhận thức", color="red",
        priority=5, severity_score=0.9, action_bias="REDUCE_RISK",
    ),
}
...
LCI_LABELS: dict[str, PresentationLabel] = {
    ...
    "CO_CUM_VUA": PresentationLabel(
        ...
        color="yellow", icon="alert-triangle",
        priority=3, severity_score=0.45,
        action_bias="WAIT",
    ),
    "CO_CUM_MANH": PresentationLabel(
        label_vi="Thanh khoản co cụm mạnh",
        color="orange", icon="trending-up",
        priority=4, severity_score=0.70,
        action_bias="REDUCE_RISK",
    ),
    "CO_CUM_CUC_DOAN": PresentationLabel(
        label_vi="Thanh khoản co cụm cực đoan",
        color="red", icon="shield-off",
        priority=5, severity_score=0.90,
        action_bias="REDUCE_RISK",
    ),
}
