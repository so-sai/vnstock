...
def build_normalized_payload(items: List[dict],
                              signal_field: str,
                              signal_schema: Dict[str, dict],
                              model_class: type) -> NormalizedPayload:
...
ABSORPTION_PHASES: dict[str, dict[str, Any]] = {
    "PANIC": {
        ...
        "en": {
            "name": "Panic",
            "tooltip": "SDI above panic threshold. Market in uncontrolled sell-off."
        }
    },
    "ABSORPTION_ACTIVE": {
        "color_code": "#FF8C00",
    ...
    "EQUILIBRIUM": {
        ...
        "vi": {
            "name": "Cân bằng",
            "tooltip": "SDI hội tụ dưới ngưỡng, volume profile xác nhận. Thị trường đã hấp thụ xong."
        },
        "en": {
            "name": "Equilibrium",
    ...
    "MONITORING": {
        ...
        "vi": {
            "name": "Giám sát",
            "tooltip": "Không có panic, thị trường vận hành bình thường. Chờ tín hiệu mới."
        },
        "en": {
            "name": "Monitoring",
            "tooltip": "No panic detected, market operating normally. Awaiting new signals."
        }
    },
...
REGIME_STATES: dict[str, dict[str, Any]] = {
    "TRENDING": {
        ...
        "vi": {
            "name": "Xu hướng rõ",
            "tooltip": "Thị trường có xu hướng tăng/giảm rõ rệt, ADX cao, độ rộng lan tỏa."
        },
        "en": {
            "name": "Trending",
            "tooltip": "Clear trend direction, high ADX, broad participation."
        }
    },
    "RANGING": {
        "color_code": "#FFD700",
        "vi": {
            "name": "Đi ngang",
            "tooltip": "Thị trường dao động trong biên độ hẹp, không có xu hướng rõ."
        },
        "en": {
            "name": "Ranging",
            "tooltip": "Market oscillating in a narrow band, no clear trend."
        }
    },
    "CRISIS": {
        "color_code": "#FF4444",
        "vi": {
            "name": "Khủng hoảng",
            "tooltip": "Thị trường suy yếu nghiêm trọng, độ rộng thấp, biến động cao."
        },
        "en": {
            "name": "Crisis",
            "tooltip": "Severe market weakness, low breadth, high volatility."
        }
    },
...
SIGNAL_V1_CLASSES: dict[str, dict[str, Any]] = {
    ...
    "RS-High": {
        ...
        "en": {
            "name": "RS High",
            "tooltip": "Stock with highest RS Rating in CRISIS market conditions. Potential leader."
        }
    },
}


def localize_signal(canonical_key: str, schema: dict[str, dict[str, Any]],
                    lang: str = "vi") -> str:
...
def to_hci(metric: str, value: Any, signal: str,
