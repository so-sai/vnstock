...
def _classify_habitat(rsi_low: float, rsi_high: float, rsi_current: float, prev_low: float, prev_high: float) -> dict:
...
def analyze_rsi_regime(
    symbol: str, lookback_days: int = LOOKBACK_DAYS, target_date: str | None = None, preloaded_df: pd.DataFrame | None = None
) -> dict:
    if preloaded_df is not None:
        ...
        with get_connection() as conn:
            ...
            df = pd.read_sql(
                f"SELECT date, close FROM daily_ohlcv WHERE symbol = ? {date_filter} ORDER BY date DESC LIMIT {lookback_days}",
                conn,
                params=(symbol,),
            )
    ...
    prev_window = (
        rsi_series[-min(120, len(rsi_series)) : -min(60, len(rsi_series))]
        if len(rsi_series) >= 120
        else rsi_series[: len(recent)]
    )
...
def _compute_weekly_rsi(
    symbol: str, lookback_days: int = 180, target_date: str | None = None, preloaded_df: pd.DataFrame | None = None
) -> dict:
    if preloaded_df is not None:
        sym_df = preloaded_df[preloaded_df["symbol"] == symbol].copy()
        if target_date:
...
def scan_market_rsi_regime(
    symbols: list | None = None, target_date: str | None = None, preloaded_df: pd.DataFrame | None = None
) -> list:
    if symbols is None:
...
def generate_market_rsi_report(target_date: str | None = None, preloaded_df: pd.DataFrame | None = None) -> dict:
    ...
    for r in bull_symbols[:5]:
        ...
        print(
            f"  {r['symbol']:5s} | RSI={r['rsi_current']:5.1f} | "
    ...
    report = {
        ...
        "alignment": {
            ...
            "warnings": [
                ...
                for r in results
                if not r.get("timeframe_alignment", {}).get("aligned")
            ],
        },
    }
...
def run_analysis(target_symbol: str | None = None) -> dict:
    if target_symbol:
        ...
        if result.get("status") == "OK":
            ...
            print(
                f"  Weekly: RSI={result.get('rsi_weekly', {}).get('rsi', 'N/A')} "
