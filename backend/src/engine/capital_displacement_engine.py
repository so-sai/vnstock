...
def scan_liquidity_concentration(target_date=None, offline: bool = False):
    ...
    if offline:
        logger.warning("[CAPDISP] offline=True → bỏ qua fetch mạng (trả OFFLINE_SKIPPED).")
        return {
            "date": today,
            "classification": "OFFLINE_SKIPPED",
            "conviction": "ZERO",
            "signals": [],
            "note": "Capital Displacement chưa có đường cache offline; bỏ qua trong EOD offline.",
        }
    logger.info("=" * 70)
    ...
    try:
        ...
        if vni_df is not None and len(vni_df) > 0:
            ...
            vni_info = {
                "close": float(last["close"]),
                "chg": round(
                    (float(last["close"]) - float(vni_df.iloc[-2]["close"])) / float(vni_df.iloc[-2]["close"]) * 100, 2
                )
                if len(vni_df) > 1
                else 0,
                "vol": int(last["volume"]),
                "vol_ratio": round(int(last["volume"]) / vni_df["volume"].tail(21).iloc[:-1].mean(), 2)
                if len(vni_df) >= 21
                else 0,
    ...
    logger.info(
        "Top1: %s=%s%% | Top3: %s%% | Top10: %s%%",
        market_concentration["top1"]["symbol"],
        market_concentration["top1"]["pct"],
        market_concentration["top3_pct"],
        top10_pct,
    )
    ...
    logger.info(
        "VNINDEX: %s (%s%%) | Vol ratio: %sx",
        vni_info.get("close", 0),
        f"{vni_info.get('chg', 0):+.2f}",
        vni_info.get("vol_ratio", 0),
    )
    logger.info("=" * 70)
