...
def evaluate_recovery_status(regime_data, velocity_5d, target_date=None):
    ...
    with get_connection() as conn:
        if target_date:
            df_idx = pd.read_sql(
                ...
                f"WHERE symbol='VNINDEX' AND date <= '{target_date}' ORDER BY date DESC LIMIT 60",
                conn,
            )
        else:
            df_idx = pd.read_sql(
                "SELECT symbol, date, close, volume FROM daily_ohlcv WHERE symbol='VNINDEX' ORDER BY date DESC LIMIT 60",
                conn,
            )
    ...
    with get_connection() as conn:
        if target_date:
            df_hist = pd.read_sql(
                f"SELECT date, status, breadth_pct FROM regime_history "
                f"WHERE date < '{target_date}' ORDER BY date DESC LIMIT 20",
                conn,
            )
        else:
    ...
    return {
        ...
        "log": log,
        "details": {"index_dd": round(current_dd, 2), "breadth_std": round(breadth_std, 2), "velocity_5d": float(velocity_5d)},
    }
