...
def calculate_leadership(lookback=60, top_n=15, min_value=1e9):
    ...
    daily_value.insert(7, 'contrib_pct',
                       np.where((daily_value['total_contrib'] != 0) & np.isfinite(daily_value['contribution']),
                                daily_value['contribution'] / daily_value['total_contrib'], 0))
    ...
    leader_stats = daily_value.groupby('symbol').agg(
    ...
    if contrib_p99 <= 0 or not np.isfinite(contrib_p99):
        contrib_p99 = 1
    leader_stats.insert(len(leader_stats.columns), 'norm_contrib',
                        np.clip(leader_stats['total_contrib'] / contrib_p99, 0, 1))

    contribution_is_positive = (leader_stats['total_contrib'] > 0).astype(float)
    leader_stats.insert(len(leader_stats.columns), 'leadership_score',
                        (leader_stats['norm_contrib'] * 0.50 +
                         leader_stats['consistency'].fillna(0) * 0.25 +
                         np.clip(leader_stats['rel_momentum'].fillna(0), 0, 1) * 0.25) * contribution_is_positive)
    ...
    for _, row in top_leaders.iterrows():
        ...
        decay_data.append({
            'symbol': sym, 'decay_score': round(decay_score, 3),
            'contrib_trend': round(slope_norm, 2), 'vol': round(vol_norm, 2),
            'corr_pen': round(corr_pen, 2), 'rs_pen': round(rs_pen, 2),
            'grade': grade
        })
    ...
    for i, (_, row) in enumerate(top_leaders.iterrows(), 1):
        ...
        print(f"{f'#{i}':<8} {row['symbol']:<6} {close_val:>8.0f} {share:>7.2f}% {row['total_contrib']:>+8.4f} {corr:>+8.3f}  {row['rel_momentum']:>+6.1%} {label:>14}")
    ...
    for _, row in decay_df.iterrows():
        print(f"{row['symbol']:<6} {row['decay_score']:>10.3f}  {row['contrib_trend']:>8.2f}  {row['vol']:>8.2f}  {row['corr_pen']:>8.2f}  {row['rs_pen']:>5.2f}  {row['grade']:>8}")
