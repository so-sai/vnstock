    ...
    def run(self, start_date="2023-01-01", end_date=None, top_n=5):
        ...
        for attempt in range(5):
            try:
                ...
                stock_query = (
                    f"SELECT symbol, date, close, adj_close, volume FROM daily_ohlcv "
                    f"WHERE symbol NOT IN ('VNINDEX', 'VN30') AND date >= '{start_date}' "
                    f"AND date <= '{end_date}'"
                )
                ...
                bench_query = (
                    f"SELECT date, high, low, adj_close as bench_close FROM daily_ohlcv "
                    f"WHERE symbol = 'VNINDEX' AND date >= '{start_date}' AND date <= '{end_date}'"
                )
        ...
        rs_matrix_baseline = (rs_comps["1m"] * 0.4 + rs_comps["3m"] * 0.3 + rs_comps["6m"] * 0.2 + rs_comps["1y"] * 0.1).rank(
            pct=True, axis=1
        ) * 100
        ...
        for i in range(1, len(all_dates)):
            ...
            if yesterday in rebalance_dates:
                ...
                weighted_rs_raw = (
                    rs_at_date["1m"] * weights["1m"]
                    + rs_at_date["3m"] * weights["3m"]
                    + rs_at_date["6m"] * weights["6m"]
                    + rs_at_date["1y"] * weights["1y"]
                ).fillna(0)
                ...
                value_current_day = (
                    pivot_value.loc[yesterday] if yesterday in pivot_value.index else pd.Series(0, index=val_yesterday.index)
                )
            ...
            for ticker in to_remove:
                ...
                if ticker in entry_dates_idx:
                    del entry_dates_idx[ticker]
        ...
        results_df.to_json(os.path.join(output_dir, "backtest_results.json"), orient="records", force_ascii=False, indent=4)

        # Shadow Log Export (V1.0.4)
        shadow_df = pd.DataFrame(shadow_log)
        if not shadow_df.empty:
            shadow_df.to_json(os.path.join(output_dir, "shadow_signals.json"), orient="records", force_ascii=False, indent=4)
