...
class StrategyCommander:
    ...
    def analyze_battle_map(self, symbols_list: list[str]) -> pd.DataFrame:
        if not symbols_list:
            raise CommanderDataError("❌ Symbols list empty.")
        market_condition: str = self.macro.get_market_condition()
        strong_stocks: list[str] = [
            s for s, d in self.rs_data.items() if isinstance(d, dict) and float(d.get("rs_rating", 0.0)) > 80.0
        ]
        ...
        for symbol in symbols_list:
            ...
            results.append(
                TacticalSignal(symbol, rs_val, rvol_val, round(foreign_acc, 2), market_condition, breadth_str, status)
            )
    ...
    def _decide_tactical_status(self, rs_score, rvol, foreign_acc, market_cond) -> str:
        if market_cond == "CAUTION":
            if rs_score > 90.0 and rvol > 1.5 and foreign_acc > 0.0:
                return "HIGH CONVICTION BUY"
            return "WATCHLIST / WAIT"
        if market_cond == "EXCELLENT":
            if rs_score > 80.0 and (rvol > 1.2 or foreign_acc > 0.0):
                return "BUY / ACCUMULATE"
            return "HOLD"
        if rs_score > 85.0 and rvol > 1.2:
            return "BUY"
        return "NEUTRAL"
