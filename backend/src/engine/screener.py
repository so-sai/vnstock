...
def compute_rs_line(df_stock, df_idx, window=63):
    ...
    merged[f"rs_{window}d"] = merged["rs_line"].pct_change(window)
    return merged
...
def scan_market(lookback=252, top_n=20, min_value=1e9):
    ...
    daily = df.groupby(["symbol", "date"], as_index=False).agg(close=("close", "last"), volume=("volume", "last"))
    ...
    for sym in liquid_symbols:
        ...
        h52_prev = (
            np.max(close_vals[-(252 + LAG) : -LAG])
            if len(close_vals) >= 252 + LAG
            else np.max(close_vals[:-LAG])
            if len(close_vals) > LAG
            else 0
        )
        ...
        results.append(
            {
                ...
                "mom_50d": mom_50d,
                "vol_trend": vt_now,
                "rs_improve": rs_improve,
                "near_high": near_high,
                "vol_stability": vol_stability,
                "rs_21d_prev": rs_21d_prev,
                "rs_63d_prev": rs_63d_prev,
                "mom_5d_prev": mom_5d_prev,
                "mom_20d_prev": mom_20d_prev,
                "mom_50d_prev": mom_50d_prev,
                "vol_trend_prev": vt_prev,
                "rs_improve_prev": rs_improve_prev,
                "near_high_prev": near_high_prev,
                "ma_bull": ma_bull,
                ...
                "breakout_50d": bo50,
                "rs_streak": rs_streak,
                "vol_consistency_pct": vol_consistency_pct,
            }
        )
    ...
    [c + "_prev" for c in ["rs_21d", "rs_63d", "mom_5d", "mom_20d", "mom_50d", "vol_trend", "rs_improve"]]
    ...
    factor_cols = ["rs_21d", "rs_63d", "mom_5d", "mom_20d", "mom_50d", "vol_trend", "rs_improve", "near_high"]
    ...
    rdf["composite"] = (
        ...
        + rdf["rs_63d_pct"] * 0.15
        + rdf["mom_5d_pct"] * 0.10
        + rdf["mom_20d_pct"] * 0.15
        + rdf["mom_50d_pct"] * 0.10
        + rdf["vol_trend_pct"] * 0.10
        + rdf["rs_improve_pct"] * 0.12
        + rdf["near_high_pct"] * 0.08
    )
    ...
    rdf["composite_prev"] = (
        rdf["rs_21d_prev_pct"] * 0.20
        + rdf["rs_63d_prev_pct"] * 0.15
        + rdf["mom_5d_prev_pct"] * 0.10
        + rdf["mom_20d_prev_pct"] * 0.15
        + rdf["mom_50d_prev_pct"] * 0.10
        + rdf["vol_trend_prev_pct"] * 0.10
        + rdf["rs_improve_prev_pct"] * 0.12
        + rdf["near_high_prev_pct"] * 0.08
    )
    ...
    def confirm_label(row):
        ...
        if row["rs_streak"] >= 10:
            score += 2
        elif row["rs_streak"] >= 5:
            score += 1
        if row["vol_consistency_pct"] >= 0.6:
            score += 2
        elif row["vol_consistency_pct"] >= 0.4:
            score += 1
        if row["ma_bull"] == 1:
            score += 1
        if row["breakout_20d"] >= 0:
            score += 1.5
        elif row["breakout_20d"] >= -0.02:
            score += 0.5
        if row["sector_ratio"] >= 0.3:
            score += 1
        if score >= 5:
            return "MANH"
        if score >= 3:
    ...
    print(
        f"{'#':<4} {'Ma':<7} {'Gia':>9} {'RS 1T':>7} {'RS 3T':>7} {'RS 6T':>7} {'Mom20':>7} {'Vol':>6} {'Diem':>6} {'Pha':>12}"
    )
    print("-" * 80)
    for _, row in dandat.iterrows():
        phase = phase_label(row["composite"])
        print(
            f"{row['rank']:<4} {row['symbol']:<7} {row['close']:>8.0f} {row['rs_21d']:>+6.1%} "
            f"{row['rs_63d']:>+6.1%} {row['rs_126d']:>+6.1%} {row['mom_20d']:>+6.1%} "
            f"{row['vol_trend']:>+5.1%} {row['composite']:>5.2f} {phase:>12}"
        )

    if not rising.empty:
        print(f"\n{'#' * 4} {'🔥 MỚI NỔI - Cải thiện hạng nhanh nhất':<60}")
        print(
            f"{'#':<4} {'Ma':<7} {'Gia':>9} {'Thay doi':>9} {'RS 1T':>7} "
            f"{'RS truoc':>8} {'Chuoi RS':>8} {'Vol deu':>7} {'Diem':>6} {'XN':>6}"
        )
        print("-" * 75)
        for _, row in rising.iterrows():
            vol_lbl = f"{row['vol_consistency_pct'] * 100:.0f}%"
            print(
                f"{row['rank']:<4} {row['symbol']:<7} {row['close']:>8.0f} {int(row['rank_change']):>+8} "
                f"{row['rs_21d']:>+6.1%} {row['rs_21d_prev']:>+6.1%} {int(row['rs_streak']):>7} "
                f"{vol_lbl:>6} {row['composite']:>5.2f} {row['xac_nhan']:>6}"
            )

    if not falling.empty:
        print(f"\n{'#' * 4} {'❄️ SUY YẾU - Tụt hạng nhiều nhất':<60}")
        print(
            f"{'#':<4} {'Ma':<7} {'Gia':>9} {'Thay doi':>9} {'RS 1T':>7} {'Chuoi RS':>8} {'Vol deu':>7} {'Diem':>6} {'XN':>6}"
        )
        print("-" * 65)
        for _, row in falling.iterrows():
            ...
            print(
                f"{row['rank']:<4} {row['symbol']:<7} {row['close']:>8.0f} {int(row['rank_change']):>+8} "
                f"{row['rs_21d']:>+6.1%} {int(row['rs_streak']):>7} {vol_lbl:>6} "
                f"{row['composite']:>5.2f} {row['xac_nhan']:>6}"
            )
