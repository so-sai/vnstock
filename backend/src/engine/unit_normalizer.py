...
class UnitNormalizer:
    ...
    def update_macro_data(self, variable_name: str, value: float, date: str | None = None) -> None:
        ...
        if self.show_log:
            print(f"✅ Macro updated: {variable_name} = {value} ({date})")

    def get_macro_trend(self, variable_name: str, window: int = 5) -> str:
        with get_connection() as conn:
            query: str = (
                f"SELECT value FROM macro_history WHERE variable = '{variable_name}' ORDER BY date DESC LIMIT {window}"
            )
            cursor = conn.cursor()
            ...
            if variable_name == "OMO":
                return "BULLISH" if current > 0.0 else "BEARISH"
            elif variable_name in ["INTERBANK_ON", "USD_VND"]:
                return "BEARISH" if current > avg else "BULLISH"
            return "NEUTRAL"
