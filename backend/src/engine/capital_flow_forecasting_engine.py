...
def _fetch_regime_history(lookback_days: int = 120, target_date: str | None = None) -> pd.DataFrame:
    with get_connection() as conn:
        ...
        df = pd.read_sql(
            f"SELECT date, regime_score, status, breadth_pct FROM regime_history {date_filter} ORDER BY date", conn
        )
    return df
...
def _compute_velocity_and_acceleration(vectors: list) -> list:
    ...
    for i in range(1, len(sorted_vectors)):
        ...
        for s in CORE_SECTORS:
            curr.sector_velocity[s] = round(curr.sector_shares.get(s, 0) - prev.sector_shares.get(s, 0), 2)

    for i in range(2, len(sorted_vectors)):
        prev_v = sorted_vectors[i - 1]
        curr_v = sorted_vectors[i]
        for s in CORE_SECTORS:
            curr_v.sector_acceleration[s] = round(curr_v.sector_velocity.get(s, 0) - prev_v.sector_velocity.get(s, 0), 3)
    ...
    for i in range(1, len(sorted_vectors)):
        ...
        curr_rank = {s: idx for idx, (s, _) in enumerate(sorted(sorted_vectors[i].sector_shares.items(), key=lambda x: -x[1]))}
        rank_changes = []
        prev_ranked = [s for s, _ in sorted(sorted_vectors[i - 1].sector_shares.items(), key=lambda x: -x[1])]
        for idx, s in enumerate(prev_ranked):
            curr_idx = curr_rank.get(s, len(prev_ranked))
            rank_changes.append(abs(idx - curr_idx))
        sorted_vectors[i].rotation_velocity = round(sum(rank_changes) / len(rank_changes), 2)
...
def _project_regime(regime_df: pd.DataFrame, flow_velocity: float, breadth_pct: float) -> dict:
    states = ["TRENDING", "RANGING", "CRISIS"]
...
def _classify_sector_trajectory(velocity: float, acceleration: float, persistence: float) -> tuple:
...
def generate_flow_forecast(target_date: str | None = None, lookback_days: int = 60) -> dict:
    ...
    logger.info("Regime Forecast: %s (confidence: %s)", regime_forecast["projected_regime"], regime_forecast["confidence"])
    ...
    for s in sorted(sector_forecasts, key=lambda x: x["velocity"], reverse=True):
        ...
        logger.info(
            "  %s %s share=%5.1f%% vel=%+.1f acc=%+.2f → %s (%s)",
            sec_name,
            arrow,
            s["current_share_pct"],
            s["velocity"],
    ...
    logger.info(
        "Summary: ACC=%s CONT=%s STABLE=%s DECEL=%s LAG=%s",
        proj["acceleration_count"],
        proj["continuation_count"],
        proj["stable_count"],
        proj["deceleration_count"],
        proj["lag_count"],
    )
    logger.info("=" * 70)
