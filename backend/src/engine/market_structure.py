...
def analyse_market_structure(lookback=60, top_n=10, min_value=1e9, verbose=True, target_date=None):
    if target_date:
    ...
    sbmi_daily = (
        ex_top.groupby("date")
        .apply(
            lambda g: (g["return"] * g["weight"]).sum() / g["weight"].sum() if g["weight"].sum() > 0 else 0.0,
