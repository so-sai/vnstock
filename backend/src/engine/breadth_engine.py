...
def run_breadth_analysis(target_date: str | None = None):
    ...
    if ref_rows.empty or ref_rows["volume"].sum() == 0:
        ...
        print(
            f"ℹ️ Ngày {latest_date.strftime('%Y-%m-%d')} không có dữ liệu "
    ...
    consistency_count = 0
    for d in prev_dates:
        df[(df["date"] == d) & (df["volume"].rolling(20).mean() >= liquidity_threshold)]
        # Re-calc for specific date if needed, but we assumed vectorized is enough
        d_nh10 = int(df[(df["date"] == d) & (df["is_nh10"])]["symbol"].nunique())
        if d_nh10 > 50:
            consistency_count += 1
