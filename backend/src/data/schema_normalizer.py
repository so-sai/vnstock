...
OHLCV_ALIASES: dict[str, list[str]] = {
    ...
    "close": ["close", "close_price", "dong_cua", "CLOSE", "Close", "CLS", "c", "last"],
    "adj_close": ["adj_close", "adjClose", "adjclose", "adjusted_close", "dieu_chinh", "ADJ_CLOSE", "adj"],
    "volume": ["volume", "total_trades", "khoi_luong", "VOLUME", "Volume", "Vol", "vol", "v", "VolumeWeighted"],
}

MACRO_ALIASES: dict[str, list[str]] = {
    "date": ["date", "Date", "DATE", "time", "Time", "TIME", "ngay", "t"],
    "symbol": ["symbol", "Symbol", "SYMBOL", "ticker", "Ticker", "ma_cp", "s", "Ticker"],
    "net_profit": [
        "netProfit",
        "net_profit",
        "LNST",
    ...
    "revenue": ["revenue", "doanh_thu", "DT", "Revenue", "total_revenue", "TotalRevenue", "sales"],
    "eps": ["eps", "EPS", "earnings_per_share", "thu_nhap_moi_co_phieu", "EarningsPerShare"],
    "roe": ["roe", "ROE", "return_on_equity", "ReturnOnEquity", "ty_suat_loi_nhuan_von"],
    "roa": ["roa", "ROA", "return_on_assets", "ReturnOnAssets"],
    "pe": ["pe", "PE", "P/E", "price_to_earnings", "PriceToEarnings", "PER"],
    "pb": ["pb", "PB", "P/B", "price_to_book", "PriceToBook", "PBR"],
    "dividend_yield": ["dividend_yield", "DividendYield", "ty_le_co_tuc", "dividendYield"],
    "market_cap": ["market_cap", "MarketCap", "marketCapitalization", "von_hoa", "marketCap", "capitalization"],
    "foreign_buy_vol": ["foreign_buy_volume", "foreign_buy_vol", "ForeignBuyVol", "foreign_vol"],
...
def get_safe(row: dict[str, Any], ptd_field: str, default: Any = None, coerce: type | None = None) -> Any:
...
def _get_adjusted_reference(symbol: str, ref_date: str) -> float | None:
    ...
    window = [e for e in events if e["ex_date"] >= _date_shift(ref_date, -30) and e["ex_date"] <= _date_shift(ref_date, 1)]
    if not window:
        return p
    ...
    for ev in sorted(window, key=lambda x: x["ex_date"]):
        p_adj = adjust_reference_price(p_adj, ev["action_type"], ev["ratio"], ev["cash_per_share"])

    if abs(p_adj - p) / max(p, 1) > 0.001:
        logger.info(f"[CA_ADJ] {symbol} @ {ref_date}: {len(window)} sự kiện → {p:.2f} → {p_adj:.2f}")
    return p_adj
...
def auto_scale_ohlcv(
        ...
        if abs(ratio / scale - 1.0) < SCALE_TOLERANCE:
            ...
            for fld in OHLCV_FLOAT_FIELDS:
                ...
                if v is not None and isinstance(v, (int, float)):
                    scaled[fld] = v * scale
            logger.info(
                f"{src_tag}{symbol}: auto_scale phát hiện lệch đơn vị ×{scale:.6f} (ref={ref_close}, incoming={close})"
            )
    ...
    if ratio > 100 or ratio < 0.01:
        logger.warning(
            f"{src_tag}{symbol}: tỷ lệ lệch {ratio:.4f} không khớp KNOWN_SCALE nào — ref={ref_close}, incoming={close}"
        )
...
def normalize_to_ptd_schema(
    ...
    if sym and normalized.get("close") is not None:
        scaled_row, scale_factor, was_scaled = auto_scale_ohlcv(normalized, str(sym).upper(), source_label, ref_date)
        if was_scaled:
...
def validate_ohlcv(
    ...
    if ref is not None and ref > 0 and close is not None and close > 0:
        ratio = ref / close
        # Nếu ratio lệch khỏi tất cả KNOWN_SCALES → scale bất thường
        matched = any(abs(ratio / s - 1.0) < SCALE_TOLERANCE * 2 for s in KNOWN_SCALES)
        if not matched and (ratio > 50 or ratio < 0.02):
            issues.append(f"scale bất thường: ref={ref}, close={close}, ratio={ratio:.2f} — không khớp KNOWN_SCALE nào")
        # Dù đã scale, kiểm tra nếu giá quá thấp (sót scale)
        if close < 1.0 and not any(abs(ref / (close * s) - 1.0) < SCALE_TOLERANCE for s in KNOWN_SCALES):
            issues.append(f"close={close} quá thấp — nghi ngờ sót scale (ref={ref})")
