...
C = {
    ...
    # COMPOUNDER chain
    "IT_BACKLOG": EngineComponent("IT_BACKLOG", "IT Outsourcing Backlog", "Hợp đồng CNTT chưa thực hiện (leading indicator)"),
    "IT_HEADCOUNT": EngineComponent("IT_HEADCOUNT", "Headcount Billable", "Số nhân sự tính phí (thước đo năng lực cung ứng)"),
    "IT_REVENUE": EngineComponent("IT_REVENUE", "Revenue Outsourcing", "Doanh thu từ outsourcing & giải pháp CNTT"),
    "IT_MARGIN": EngineComponent("IT_MARGIN", "Operating Margin", "Biên lợi nhuận từ mảng CNTT (cao nhờ switching cost)"),
    "DOMESTIC_IT": EngineComponent(
        "DOMESTIC_IT", "Domestic IT Revenue", "Doanh thu thị trường nội địa (chính phủ, ngân hàng)"
    ),
    "EDUCATION_REV": EngineComponent("EDUCATION_REV", "Education Revenue", "Doanh thu giáo dục (phân khúc thấp hơn)"),
    # CYCLICAL_HEAVY chain
    "CAPACITY_UTIL": EngineComponent(
        "CAPACITY_UTIL", "Capacity Utilization", "Tỷ lệ vận hành công suất (leading indicator)", unit="%", is_leading=True
    ),
    "STEEL_SPREAD": EngineComponent(
        "STEEL_SPREAD", "Steel Spread (HRC - Raw)", "Chênh lệch giá thép - nguyên liệu đầu vào", unit="VND/ton"
    ),
    "INVENTORY_VALUE": EngineComponent("INVENTORY_VALUE", "Inventory Revaluation", "Chênh lệch giá trị tồn kho (LIFO/FIFO)"),
    "GROSS_MARGIN": EngineComponent("GROSS_MARGIN", "Gross Margin", "Biên lãi gộp (chịu ảnh hưởng spread + inventory reval)"),
    "EBITDA_MARGIN": EngineComponent("EBITDA_MARGIN", "EBITDA Margin", "Biên EBITDA (đòn bẩy hoạt động cao)"),
    # BANK chain
    "CASA_RATIO": EngineComponent(
        "CASA_RATIO", "CASA Ratio", "Tỷ lệ tiền gửi không kỳ hạn (chi phí vốn thấp)", unit="%", is_leading=True
    ),
    "FUNDING_COST": EngineComponent("FUNDING_COST", "Funding Cost", "Chi phí huy động vốn bình quân", unit="%"),
    "LOAN_BOOK": EngineComponent("LOAN_BOOK", "Loan Book Growth", "Tăng trưởng dư nợ tín dụng", unit="%"),
    "NIM": EngineComponent("NIM", "Net Interest Margin", "Chênh lệch lãi suất đầu vào - đầu ra", unit="%"),
    "NPL": EngineComponent("NPL", "Non-Performing Loan Ratio", "Tỷ lệ nợ xấu", unit="%"),
    "PPOP": EngineComponent("PPOP", "Pre-Provision Operating Profit", "Lợi nhuận trước dự phòng (core earning power)"),
    "FEE_INCOME": EngineComponent("FEE_INCOME", "Fee Income (bancassurance)", "Thu nhập phí từ bảo hiểm, thanh toán,..."),
    # REAL_ESTATE chain
    "LAND_BANK": EngineComponent("LAND_BANK", "Land Bank (Ha)", "Quỹ đất sạch chưa phát triển"),
    "PRESALES": EngineComponent("PRESALES", "Presales Value", "Giá trị hợp đồng bán trước (leading)", is_leading=True),
    "HANDOVER_REV": EngineComponent("HANDOVER_REV", "Handover Revenue", "Doanh thu bàn giao (ghi nhận doanh thu)"),
    "RECEIVABLES": EngineComponent("RECEIVABLES", "Receivables (installment)", "Phải thu khách hàng trả góp"),
    # RETAIL chain
    "STORE_COUNT": EngineComponent("STORE_COUNT", "Store Count", "Số lượng cửa hàng"),
    "SSS": EngineComponent(
        "SSS", "Same-Store-Sales Growth", "Tăng trưởng doanh thu cùng cửa hàng (leading)", unit="%", is_leading=True
    ),
    "RETAIL_MARGIN": EngineComponent("RETAIL_MARGIN", "Retail Margin", "Biên lãi gộp bán lẻ"),
    "INVENTORY_TURN": EngineComponent("INVENTORY_TURN", "Inventory Turnover", "Vòng quay hàng tồn kho"),
    # REIT / Commercial RE chain
    "OCCUPANCY": EngineComponent(
        "OCCUPANCY", "Occupancy Rate", "Tỷ lệ lấp đầy TTTM/văn phòng (leading)", unit="%", is_leading=True
    ),
    "RENTAL_YIELD": EngineComponent(
        "RENTAL_YIELD", "Rental Yield", "Suất sinh lời cho thuê trên m² (leading)", unit="%", is_leading=True
    ),
    "LEASE_REVENUE": EngineComponent("LEASE_REVENUE", "Lease Revenue", "Doanh thu cho thuê định kỳ"),
    # UTILITY chain
    "GAS_VOLUME": EngineComponent("GAS_VOLUME", "Gas Volume Sold", "Sản lượng khí tiêu thụ", is_leading=True),
    "GAS_TARIFF": EngineComponent("GAS_TARIFF", "Regulated Tariff", "Giá khí do nhà nước điều tiết"),
    "BRENT_LINK": EngineComponent(
        "BRENT_LINK", "Brent-linked Gas Price", "Giá khí điều chỉnh theo Brent (cơ chế pass-through)"
    ),
    "UTILITY_EBITDA": EngineComponent("UTILITY_EBITDA", "EBITDA (Utility)", "EBITDA từ hạ tầng khí (ổn định, biên cao)"),
}
...
_reg_chain(
    "COMPOUNDER",
    "Công ty Tăng trưởng Chất lượng Cao",
    [
        "IT_BACKLOG",
        "IT_HEADCOUNT",
        "IT_REVENUE",
...
_reg_chain(
    "REIT_COMMERCIAL",
    "Công ty Cho thuê BĐS Thương mại",
    [
        "OCCUPANCY",
        "RENTAL_YIELD",
        "LEASE_REVENUE",
...
class EconomicEngine:
    ...
    def get_chain(self, symbol: str) -> PropagationChain | None:
    ...
    def get_chain_by_archetype(self, archetype: str) -> PropagationChain | None:
...
def print_engine_report(results: dict[str, list]):
    print(f"\n  {'=' * 80}")
    print("  ECONOMIC ENGINE — CHAIN OF CAUSATION")
    print(f"  {'=' * 80}")
