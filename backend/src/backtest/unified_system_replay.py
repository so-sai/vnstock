...
def _compute_scores_for_date(conn, target_date, macro_engine, lag_engine, ix_engine, matrix, macro_cache):
    ...
    if macro_cache and macro_cache["date"] == target_date:
        ...
        except Exception:  # noqa: BLE001 - batch isolation: 1 ngày macro lỗi không dừng replay
            macro_result = None
    ...
    for sym in UNIVERSE:
        ...
        except Exception:  # noqa: BLE001 - batch isolation: 1 ngành lỗi không dừng toàn bộ
            scores_a[sec] = 0.5
    ...
    try:
        ...
        try:
            ...
            if cached_version != current_version:
                return False
        except Exception:  # noqa: BLE001 - cache fallback: metadata hỏng → recompute (không crash replay)
            return False
...
def run_unified_replay(
    ...
    if _macro_cache_valid(CACHE_FILE, db_path, start_date):
        ...
        for d in scored_dates:
            ...
            except Exception:  # noqa: BLE001 - batch isolation: 1 ngày macro lỗi không dừng pre-fetch
                macro_cache[d] = None
        ...
        for d in scored_dates:
            ...
            except Exception:  # noqa: BLE001 - batch isolation: 1 ngày lỗi không dừng pre-fetch
        ...
        except Exception as e:  # noqa: BLE001 - cache fallback: ghi Parquet hỏng thì bỏ qua (không crash)
...
def run_multi_factor_backtest(start="2021-04-01", end="2026-08-04", db_path=None, sample_every=5):
    ...
    except Exception:  # noqa: BLE001 - preload cache tùy chọn: hỏng thì bỏ qua (không crash)
        logger.debug("[REPLAY] Parquet preload bỏ qua (cache không dùng được)")
    ...
    for i, date in enumerate(dates):
        ...
        if date in score_days:
            ...
            except Exception as exc:  # noqa: BLE001 - resilience: log mọi lỗi engine để không tạo backtest 0 lệnh âm thầm
