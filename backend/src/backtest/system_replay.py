...
# Fix Unicode encoding for Windows console (charmap can't encode Greek α)
        ...
        for trade in self.active_trades:
            ...
            if not trade["trailing_active"] and ret >= 4.0:
                ...
                trade["trailing_stop"] = trade["entry_price"] * (
                    1 + (ret - max(2 * atr14 / trade["entry_price"] * 100, 1.5)) / 100
                )
...
def run_stress_test(start_date, end_date):
    ...
    with get_connection() as conn:
        sql = (
            ...
            "WHERE symbol='VNINDEX' AND date BETWEEN "
            f"'{start_date}' AND '{end_date}' ORDER BY date"
        )
        df_days = pd.read_sql(sql, conn)
    ...
    for i, target_date in enumerate(trading_days):
        ...
        try:
            ...
            with get_connection() as conn:
                current_idx = pd.read_sql(
                    f"SELECT close FROM daily_ohlcv WHERE symbol='VNINDEX' AND date = '{target_date}'", conn
                ).iloc[0]["close"]
...
if __name__ == "__main__":
    ...
    run_stress_test(args.start, args.end)
