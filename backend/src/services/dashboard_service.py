...
def get_dashboard_data() -> dict:
    ...
    try:
        ...
        if os.path.exists(portfolio_path):
            ...
            total_position_value = sum(p.get("quantity", 0) * p.get("entry_price", 0) * 1000 for p in positions)
...
def _generate_system_message(regime_status: str, risk_level: str) -> dict:
    ...
    return mapping.get(
        regime_status,
        {
            ...
            "advise": (
                "Hệ thống đang thu thập thêm dữ liệu độ rộng. Duy trì tỷ trọng danh mục ở mức cân bằng, chờ tín hiệu xác nhận."
            ),
            "color": "gray",
        },
    )
