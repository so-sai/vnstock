...
from src.alpha.forward_return_tagger import ForwardReturnRow
...
class RegimeReturnStats:
    ...
    # t+5 stats
    mean_fwd5: float | None
    std_fwd5: float | None
    hit_rate_fwd5: float | None  # % days with fwd_ret_t5 > 0
    sharpe_fwd5: float | None  # mean / std (not annualized)
    worst_fwd5: float | None
    best_fwd5: float | None

    # t+20 stats
    mean_fwd20: float | None
    std_fwd20: float | None
    hit_rate_fwd20: float | None
    sharpe_fwd20: float | None
    worst_fwd20: float | None
    best_fwd20: float | None

    # t+1 stats (overnight edge check)
    mean_fwd1: float | None
    hit_rate_fwd1: float | None
...
def compute_regime_bias_score(matrix: list[RegimeReturnStats]) -> float:
    ...
    if total_weight == 0:
        return 0.0
    weighted_sum = sum((s.mean_fwd5 or 0.0) * s.n_days for s in matrix if s.mean_fwd5 is not None)
