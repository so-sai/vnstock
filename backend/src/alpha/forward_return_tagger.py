...
class ForwardReturnRow:
    ...
    trend_score: float
    vol_score: float
    fwd_ret_t1: float | None  # (close[t+1] - close[t]) / close[t]
    fwd_ret_t5: float | None  # (close[t+5] - close[t]) / close[t]
    fwd_ret_t20: float | None  # (close[t+20] - close[t]) / close[t]
...
def build_tagged_rows(
    ...
    with get_connection() as conn:
        ...
        vnindex_df = pd.read_sql("SELECT date, close FROM daily_ohlcv WHERE symbol = 'VNINDEX' ORDER BY date", conn)
    ...
    for _, row in regime_df.iterrows():
        ...
        def _ret(future_close) -> float | None:
