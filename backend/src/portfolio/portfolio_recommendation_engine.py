...
def _get_regime_from_db() -> dict:
    try:
        with get_connection() as conn:
            df = pd.read_sql(
                "SELECT date, status, regime_score, breadth_pct FROM regime_history ORDER BY date DESC LIMIT 1", conn
            )
        if not df.empty:
            return {
...
def _score_symbol(
    symbol: str, sector: str, rs_data: pd.DataFrame, forecast: dict, displacement: dict, regime: dict, rsi_report: dict
) -> dict:
...
def generate_recommendations(target_date: str | None = None) -> dict:
    ...
    for tier_name in ["core", "rotation", "opportunity"]:
        ...
        for r in recommendations[tier_name]:
            ...
            print(
                f"  {arrow} {r['symbol']:5s} | {r['conviction']:3.0f}% | "
