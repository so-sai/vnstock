...
def arbitrate(model_a_verdict: str, model_b_verdict: str,
              active_regime: str) -> dict:
    policy = REGIME_POLICY.get(active_regime)
    if not policy:
        return {"action": "STAND_DOWN", "reason": "UNKNOWN_REGIME",
                "target_engine": None, "confidence_multiplier": 0.0}

    if policy["action"] == "FORCE_CASH":
        return {"action": "FORCE_CASH", "reason": "REGIME_CRISIS_LOCKDOWN",
                "target_engine": None, "confidence_multiplier": 0.0}
    ...
    if policy["action"] == "CONSENSUS_ONLY":
        if a_buy and b_buy:
            return {"action": "EXECUTE", "reason": "BOTH_CONFIRM_RECOVERY",
                    "target_engine": "BOTH", "confidence_multiplier": 1.2}
        return {"action": "STAND_DOWN", "reason": "RECOVERY_NEEDS_BOTH",
                "target_engine": None, "confidence_multiplier": 0.0}
