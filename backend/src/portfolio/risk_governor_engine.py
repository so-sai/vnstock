    ...
    try:
        with get_portfolio_connection() as conn:
            ...
            telemetry = pd.read_sql(
                "SELECT * FROM portfolio_telemetry ORDER BY date DESC LIMIT 1",
                conn
            )
    except:
...
def compute_drawdown() -> dict:
    try:
        with get_portfolio_connection() as conn:
            nav_series = pd.read_sql(
                "SELECT date, total_equity FROM portfolio_telemetry ORDER BY date",
                conn
            )
    except:
...
def compute_var(symbols: list, weights: Optional[list] = None) -> dict:
    if not symbols:
...
def compute_correlation_risk(positions_df: pd.DataFrame) -> dict:
    if positions_df.empty or len(positions_df) < 2:
        return {'avg_correlation': 0.0, 'high_corr_pairs': [], 'risk_level': 'LOW',
                'high_corr_count': 0, 'correlation_limit': CORRELATION_LIMIT, 'status': 'INSUFFICIENT'}
    ...
    try:
        ...
        return {'avg_correlation': 0.0, 'high_corr_pairs': [], 'risk_level': 'LOW',
                'high_corr_count': 0, 'correlation_limit': CORRELATION_LIMIT, 'status': 'NO_DATA'}
...
def evaluate_risk_governance() -> dict:
    ...
    var_data = compute_var(
        snapshot['positions']['symbol'].tolist() if not snapshot['positions'].empty else []
    )
...
def _store_risk_verdict(verdict: dict):
    try:
        with get_connection() as conn:
            ...
            cols = ', '.join(row.keys())
            vals = ', '.join(['?'] * len(row))
            conn.execute(f"INSERT OR REPLACE INTO risk_governance_history ({cols}) VALUES ({vals})",
                         list(row.values()))
            conn.commit()
