...
def _get_current_status(position_id: int) -> str:
    with get_portfolio_connection() as conn:
        row = conn.execute(
            "SELECT status FROM position_lifecycle WHERE id=?",
            (position_id,)
        ).fetchone()
    return row[0] if row else None

def transit_status(position_id: int, next_status: str, update_data: dict = None) -> dict:
    ...
    if update_data:
        for key in ("avg_cost", "current_size", "highest_price_held",
                     "stop_loss_price", "take_profit_price", "thesis_notes",
                     "conviction_score", "initial_risk_pct", "exit_reason", "context_source",
                     "realized_pnl_pct"):
...
def create_position(symbol: str, regime_at_entry: str, entry_price: float,
                    stop_loss_price: float, thesis_source: str,
                    thesis_notes: str = None, take_profit_price: float = None,
                    conviction_score: float = 0.0, initial_risk_pct: float = 0.0,
                    context_source: str = "SECONDARY_STABLE") -> dict:
    with get_portfolio_connection() as conn:
        cursor = conn.execute("""
            ...
            VALUES (?, 'ARMED', ?, ?, ?, ?, 0, ?, ?, ?, ?, ?, ?, ?)
        """, (symbol, regime_at_entry, datetime.now().strftime("%Y-%m-%d"),
              entry_price, entry_price, stop_loss_price, take_profit_price,
              thesis_source, thesis_notes, conviction_score, initial_risk_pct, context_source))
        position_id = cursor.lastrowid
    return {"ok": True, "position_id": position_id, "symbol": symbol, "status": "ARMED"}

def arm_and_size(symbol: str, regime_at_entry: str, entry_price: float,
                 stop_loss_price: float, thesis_source: str,
                 total_nav: float, cash_balance: float,
                 signal_quality: float = 0.5, breadth_alignment: float = 0.5,
                 liquidity_rank: float = 0.5, regime_confidence: float = 0.5,
                 liquidity_20d: float = 1e9,
                 thesis_notes: str = None, take_profit_price: float = None,
                 active_sector_exposure: float = 0.0,
                 model_a_verdict: str = None, model_b_verdict: str = None) -> dict:
    ...
    if model_a_verdict and model_b_verdict:
        ...
        if fusion["action"] in ("STAND_DOWN", "FORCE_CASH", "MUTED"):
            return {"ok": False, "reason": f"FUSION: {fusion['reason']}",
    ...
    if throttle["throttle_activated"]:
        return {"ok": False, "reason": f"THROTTLE: {throttle['risk_state']}",
                "conviction_score": 0.0}

    regime_confidence = regime_confidence * dampener_result["net_dampener"] * throttle["multiplier"]

    dv = position_sizer.calculate_conviction_score(
        signal_quality, breadth_alignment, liquidity_rank, regime_confidence)
    if dv < 0.4:
        return {"ok": False, "reason": "CONVICTION_TOO_LOW", "conviction_score": dv}

    sizer_result = position_sizer.compute_position_size(
        total_nav, cash_balance, regime_at_entry,
        entry_price, stop_loss_price, symbol=symbol,
        conviction_score=dv, current_sector_exposure=active_sector_exposure)
    ...
    heat_risk_pct = (total_nav * sizer_result["initial_risk_pct"] / 100) / total_nav * 100

    ok_gate, msg_gate = risk_budget.evaluate_gatekeeper(
        regime_at_entry, 0.0, 0.0, exposure_pct)
    if not ok_gate:
        return {"ok": False, "reason": f"GATEKEEPER: {msg_gate}", "conviction_score": dv}

    create_result = create_position(
        symbol, regime_at_entry, entry_price, stop_loss_price,
        thesis_source, thesis_notes, take_profit_price)
    if not create_result.get("ok"):
        return create_result

    pid = create_result["position_id"]
    transit_result = transit_status(pid, "ENTERED", {
...
def close_position(position_id: int, exit_price: float,
                   ...
                   max_adverse_excursion: float = None) -> dict:
    ...
    if streak["streak_losses"] >= 3:
        memory_engine.log_conviction_decay(
            thesis_source, regime,
            streak["streak_losses"], 0, 0.5, 1.0,
            f"POST_CLOSE_LOSS_STREAK_{streak['streak_losses']}"
        )
...
def get_portfolio_summary() -> dict:
    with get_portfolio_connection() as conn:
        ...
        snapshot = conn.execute(
            "SELECT * FROM portfolio_telemetry ORDER BY snapshot_date DESC LIMIT 1"
        ).fetchone()
    return {
        "open_positions": active[0],
        "total_shares": active[1],
        "market_value_vnd": round(active[2], 0),
        "latest_snapshot": dict(zip(
            _get_telemetry_columns(),
            snapshot or [None]*6
