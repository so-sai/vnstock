...
def compute_v2_decayed(override_input: DecisionTensorInput | None = None) -> dict:
    ...
    except (sqlite3.Error, TypeError, ValueError, KeyError) as _e:
        logger.debug("Ghi telemetry quyết định thất bại (bỏ qua, không ảnh hưởng quyết định): %s", _e)
...
def compute_v2(override_input: DecisionTensorInput | None = None) -> dict:
    ...
    try:
        ...
        logger.debug("Đọc final_decision.json thất bại (bỏ qua): %s", _e)
    return result
...
def log_override(decision_id: str, override_action: str, override_reason: str) -> dict:
    ...
    for entry in history:
        if entry.get("decision_id") == decision_id:
            ...
            except (ImportError, AttributeError, TypeError, KeyError) as _e:
                logger.debug("Audit override thất bại (bỏ qua): %s", _e)
    ...
    for entry in history:
        if entry.get("decision_id") == decision_id:
            ...
            try:
                ...
                logger.debug("Audit confirm thất bại (bỏ qua): %s", _e)
