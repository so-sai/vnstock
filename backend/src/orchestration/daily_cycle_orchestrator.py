...
class ExecutionGraph:
    ...
    def add_node(
        ...
        self.nodes[nid] = NodeSpec(id=nid, deps=deps, stale_window=stale_window, desc=desc, fn=run)
        return self
        ...
        def visit(nid: str):
            ...
            for dep in self.nodes[nid].deps:
                if dep not in self.nodes:
                    continue
                if color[dep] == GRAY:
                    raise GraphCycleError(f"cycle detected: {dep} → {nid}")
                if color[dep] == WHITE:
                    visit(dep)
            color[nid] = BLACK
            order.append(nid)
    ...
    def run(
        self, ctx: dict | None = None, skip_fresh: bool = True, fresh_checker: Callable[[str], bool] | None = None
    ) -> dict[str, dict]:
        ctx = ctx if ctx is not None else {}
        results: dict[str, dict] = {}
        for nid in self.order():
            node = self.nodes[nid]
            if skip_fresh and node.stale_window is not None and fresh_checker is not None and fresh_checker(nid):
...
def record_run_log(scenario: str, node_id: str, status: str, meta: dict | None = None, db_path: str | None = None):
    ...
    try:
        ...
        conn.execute(
            "INSERT INTO daily_cycle_run_log (scenario, node_id, status, started_at, meta) VALUES (?,?,?,?,?)",
            (
                scenario,
                node_id,
                status,
...
def _is_fresh(scenario: str, node_id: str, window_days: int, db_path: str | None = None) -> bool:
...
SCENARIOS: dict[str, list[NodeSpec]] = {
    "morning": [
        NodeSpec("sbv_update", deps=[], stale_window=1, desc="SBV Update — interbank rates", fn=_run_sbv),
        NodeSpec("world_sensors", deps=[], stale_window=1, desc="Fed/World sensors", fn=_run_world_sensors),
        NodeSpec("macro_state", deps=["world_sensors"], stale_window=1, desc="MacroState classifier", fn=_run_macro_state),
        NodeSpec("transmission", deps=["macro_state"], stale_window=1, desc="Economic transmission", fn=_run_transmission),
        NodeSpec(
            "governor", deps=["transmission"], stale_window=1, desc="Bayesian Governor — Morning Brief", fn=_run_governor
        ),
        NodeSpec("system_audit", deps=["governor"], stale_window=None, desc="System Health Report (CRO)", fn=None),
    ],
    "close": [
        NodeSpec("market_data", deps=[], stale_window=1, desc="EOD pipeline — market data", fn=_run_market_data),
        NodeSpec("breadth", deps=["market_data"], stale_window=1, desc="Breadth engine", fn=_run_breadth),
        NodeSpec("sector", deps=["breadth"], stale_window=1, desc="Sector flow / rotation", fn=_run_sector),
        NodeSpec("governor", deps=["sector"], stale_window=1, desc="Bayesian Governor — Daily Report", fn=_run_governor),
        NodeSpec("system_audit", deps=["governor"], stale_window=None, desc="System Health Report (CRO)", fn=None),
    ],
    "earnings": [
        NodeSpec("financial_crawl", deps=[], stale_window=7, desc="Financial crawl (BCTT)", fn=_run_financial_crawl),
        NodeSpec("health_v2", deps=["financial_crawl"], stale_window=1, desc="Company Health Engine v2", fn=_run_health_v2),
        NodeSpec("valuation", deps=["health_v2"], stale_window=7, desc="Valuation Engine", fn=_run_valuation),
        NodeSpec(
            "vn20_builder",
            deps=["valuation"],
            stale_window=7,
        ...
        NodeSpec("governor", deps=["valuation"], stale_window=1, desc="Bayesian Governor", fn=_run_governor),
        NodeSpec("system_audit", deps=["governor"], stale_window=None, desc="System Health Report (CRO)", fn=None),
    ],
}
...
class DailyCycleOrchestrator:
    ...
    def __init__(self, db_path: str | None = None, scenarios: dict[str, list[NodeSpec]] | None = None):
        self.db_path = db_path or _cycle_db_path()
        self.scenarios = scenarios or SCENARIOS

    def build_graph(self, scenario: str, audit_days: int = 90, persist_audit: bool = False) -> ExecutionGraph:
        ...
        for spec in self.scenarios[scenario]:
            ...
            g.add_node(spec.id, run=fn, deps=spec.deps, stale_window=spec.stale_window, desc=spec.desc)
        return g
    ...
    def run(
        ...
        graph = self.build_graph(scenario, audit_days=audit_days, persist_audit=persist_audit)
        results = graph.run(skip_fresh=skip_fresh, fresh_checker=self._fresh_checker(scenario))

        if persist_log:
            for nid, res in results.items():
                record_run_log(scenario, nid, res.get("status", "OK"), db_path=self.db_path)
...
def print_cycle_report(report: dict):
    print(f"\n  {'=' * 100}")
    print(f"  DAILY CYCLE — {report.get('workflow', '?')} | {report.get('generated_at', '')}")
    print(f"  {'=' * 100}")
    nodes = report.get("nodes", {})
    for nid, res in nodes.items():
        icon = {"OK": "✅", "SKIPPED": "⏭", "FAILED": "❌"}.get(res.get("status"), "•")
        print(f"  {icon} {nid:<16} {res.get('status', ''):<8} {res.get('error', '')}")
    print(f"  {'─' * 100}")
...
if __name__ == "__main__":
    ...
    report = orch.run(scenario, skip_fresh=False, persist_audit=True, persist_log=True)
