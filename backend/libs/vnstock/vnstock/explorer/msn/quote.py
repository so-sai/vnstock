...
from .const import _BASE_URL, _OHLC_DTYPE, _OHLC_MAP, _RESAMPLE_MAP
...
class Quote:
    ...
    def _input_validation(self, start: str, end: str, interval: str):
        ...
        if timeframe_value not in _RESAMPLE_MAP.keys():
            msg = f"Giá trị interval không hợp lệ: {timeframe_value}. MSN chỉ hỗ trợ: 1D, 1W, 1M"
            raise ValueError(msg)

        # Create ticker model with standardized interval
        ticker = TickerModel(symbol=self.symbol_id, start=start, end=end, interval=timeframe_value)
        return ticker
