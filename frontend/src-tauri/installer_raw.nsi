  ...
  ${If} $R0 = 0
      ...
      kill:
        !if "${INSTALLMODE}" == "currentUser"
          ...
          nsis_tauri_utils::KillProcessCurrentUser "uv_backend-x86_64-pc-windows-msvc.exe"
        !else
          nsis_tauri_utils::KillProcess "${MAINBINARYNAME}.exe"
          nsis_tauri_utils::KillProcess "uv_backend-x86_64-pc-windows-msvc.exe"
        !endif
        Pop $R0
        Pop $R1
        Sleep 500
      ...
      cancel:
        Abort "$(appRunning)"
  ${Else}
      ; Nếu ứng dụng chính không chạy, diệt ngầm tiến trình zombie sidecar nếu có
      !if "${INSTALLMODE}" == "currentUser"
        nsis_tauri_utils::KillProcessCurrentUser "uv_backend-x86_64-pc-windows-msvc.exe"
      !else
        nsis_tauri_utils::KillProcess "uv_backend-x86_64-pc-windows-msvc.exe"
...
Section Uninstall
  ...
  ; Clean custom user data paths
  RMDir /r "$INSTDIR\backend"
  RMDir /r "$PROFILE\.ptck_vn"
  RMDir /r "$LOCALAPPDATA\PTCK_VN"
