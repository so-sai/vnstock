...
def measure_performance(test_name: str, threshold: float = None):
    ...
    try:
        ...
        if threshold and metrics["execution_time"] > threshold:
            pytest.warns(UserWarning, f"Test {test_name} exceeded threshold: {metrics['execution_time']:.2f}s > {threshold}s")
        ...
        for col, (min_val, max_val) in ranges.items():
            if col in df.columns:
                ...
                if col_min < min_val or col_max > max_val:
                    errors.append(f"Column {col} out of range: [{col_min}, {col_max}] not in [{min_val}, {max_val}]")
        ...
        for key, expected_type in type_map.items():
            if key in response:
                if not isinstance(response[key], expected_type):
                    errors.append(f"Key {key}: expected {expected_type}, got {type(response[key])}")
...
class TestReportGenerator:
    ...
    def __init__(self, output_dir: str = "tests/report"):
        ...
        self.report = {"timestamp": None, "test_summary": {}, "coverage": {}, "performance": {}, "quality": {}}
