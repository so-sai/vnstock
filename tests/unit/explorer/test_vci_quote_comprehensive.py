...
class TestVCIQuoteComprehensive:
    ...
    def test_history_all_intervals_hose_sample(self, diverse_test_symbols, test_intervals):
        ...
        for symbol in hose_samples:
            ...
            for interval in ["1D", "1W", "1M"]:  # Start with longer intervals
                try:
                    ...
                    assert isinstance(df, pd.DataFrame), f"Failed for {symbol} with interval {interval}"

                    if not df.empty:
                        expected_cols = ["open", "high", "low", "close"]
                        for col in expected_cols:
                            assert col in df.columns, f"Missing {col} in {symbol} {interval}"

                except Exception as e:
                    pytest.fail(f"history() failed for {symbol} interval={interval}: {e}")
    ...
    # @pytest.mark.skip(reason="Integration test - requires live API")
    def test_history_different_date_ranges(self, diverse_test_symbols, test_date_ranges):
        ...
        for range_name, dates in test_date_ranges.items():
            try:
                df = quote.history(start=dates["start"], end=dates["end"], interval="1D")
