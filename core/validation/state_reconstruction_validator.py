...
from .backtest_contract import (
    BacktestContract, RegimeEvent, parse_date, tolerance_for_event,
)
...
REQUIRED_FIELDS = [
    ...
    "breadth_health", "lcr_pct", "bdi_signal",
    "flow_bias_score", "ssi_score",
    "dcl_verdict", "dcl_score", "compensations_triggered",
]
...
def validate_snapshot(snapshot: dict) -> Optional[str]:
...
def _event_overlaps(event: RegimeEvent, ttl_detection_date: str,
...
def run_srv(
    snapshots: list[dict],
    contract: Optional[BacktestContract] = None,
) -> SRVReport:
    if contract is None:
    ...
    suite_a.mean_confidence = round(
        sum(e["bias_confidence"] for e in dbe_log) / len(dbe_log), 4
    )
    suite_a.mean_strength = round(
        sum(e["bias_strength"] for e in dbe_log) / len(dbe_log), 4
    )
    ...
    suite_b.mean_stability = round(
        sum(e["stability"] for e in dpl_log) / len(dpl_log), 4
    )
    suite_b.mean_stability_when_persistent = round(
        sum(e["stability"] for e in persistent) / max(1, len(persistent)), 4
    ) if persistent else 0.0
    suite_b.mean_stability_when_flickering = round(
        sum(e["stability"] for e in flickering) / max(1, len(flickering)), 4
    ) if flickering else 0.0
    ...
    for trigger in triggers:
        ...
        best_event: Optional[RegimeEvent] = None
        best_delay = 999
        ...
        if best_event is not None:
            ...
            suite_c.matches.append(TransitionMatch(
    ...
    suite_c.hit_rate = round(
        suite_c.hits / max(1, len(contract.regime_events)), 4
    )
    suite_c.false_positive_rate = round(
        false_positives / max(1, len(triggers)), 4
    ) if triggers else 0.0
    suite_c.mean_delay_days = round(
        sum(m.delay_days for m in suite_c.matches) / max(1, len(suite_c.matches)), 1
    ) if suite_c.matches else 0.0
